...
#include "WalkingAdaptionConfiguration.h"
...
int main(int argc, char *argv[]) {
    ...
    WalkingAdaptionConfiguration *configuration = new WalkingAdaptionConfiguration();
   ...
   try {
        ...
        VirtualRobot::RobotNodeSetPtr rns = model->getRobotNodeSet("RightLeg-7dof");
        ...
        max[0]=0.872665+1.65806,max[1]=0.785398+0.785398,max[2]=1.13446+0.349066,max[3]=2.26893,max[4]=0.698132+0.523599,max[5]=0.523599+0.523599,max[6]=0.349066+0.349066;

        if (!tmp) {


            std::vector<float> splitTimesteps = { 0.5f, 0.9f, 1.3f, 1.5f, 1.9f, 2.2f, 2.6f, 2.9f  };
            ...
            splitTimesteps.push_back(motion->getMaxTimestep());
                for (unsigned int i = 1; i < splitTimesteps.size(); i++) {
                  ...
                  //float hdpi=HDPI_calculator(result_ri,result_or),hpdi_qua=HDPI_calculator_quatenion(result_ri_qu,result_or_qu);
                  Eigen::VectorXd fpdi=FPDI_calculator(result_fp_eu,result_or_fp,max);
                  //Eigen::VectorXd fpdi=FPDI_calculator(result_fp_ri,result_or_fp,max);
