function geodesic_movement_generation02
...
% The geodesic is generated from an initial and a desired configuration. It
% is obtained by solving the geodesic ODE, aiming at finding the shortest
...
G = robot.inertia(q); % Mass-inertia matrix of the system - This is also the Riemannian metric! 
...
qd = [pi/4 ; -pi/2]; % Desired robot configuration 

% Compute geodesic solution
[ curve, logmap, len, failed, solution ] = compute_geodesic_bvp(robot, q0, qd);
...
figure('position',[10 10 900 500],'color',[1 1 1]); hold on;
left_color = [0 0 0];
right_color = [0 0 .7];
