...
p2 = R*[d1;0;0] + p1;
h = patch(x(1,:),x(2,:),x(3,:),facecol,'edgecolor',edgecol,'linewidth',2); 
msh = [sin(linspace(0,2*pi,nbSegm)); cos(linspace(0,2*pi,nbSegm))]*sz*0.2;
h = [h patch(msh(1,:)+p1(1), msh(2,:)+p1(2), repmat(p1(3),1,nbSegm), edgecol,'edgeColor',edgecol,'linewidth',2)]; 
h = [h patch(msh(1,:)+p2(1), msh(2,:)+p2(2), repmat(p2(3),1,nbSegm), edgecol,'edgeColor',edgecol,'linewidth',2)]; 
